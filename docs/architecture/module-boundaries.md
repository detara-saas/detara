...
O consentimento para avisos automáticos por WhatsApp pertence à configuração de Notificações e registra data e usuário de ativação. Testes de conexão também pertencem a Notificações: usam um tipo explícito no histórico, sem forjar Cliente ou Ordem de Serviço, e passam pelo mesmo worker e provider tenant-safe. O envio manual da OS continua autorizado pela policy própria e não é transformado em campanha ou automação de marketing.
