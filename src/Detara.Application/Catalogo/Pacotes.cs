...
public sealed record PacoteListaItemResultado(Guid Id, string Nome, int QuantidadeServicos, TipoPrecificacao TipoPrecificacao, decimal? Preco, decimal? SomaServicos, decimal? Economia, int? DuracaoEstimadaMinutos, bool EhAtivo);
...
internal sealed class AtualizarPacoteValidator : PacoteValidatorBase<AtualizarPacoteCommand> { public AtualizarPacoteValidator() { RuleFor(x => x.Id).NotEmpty(); Regras(x => x.Nome, x => x.Descricao, x => x.TipoPrecificacao, x => x.Preco, x => x.ServicoIds); } }
...
internal sealed class CriarPacoteHandler(IUsuarioContexto usuario, IServicosRepositorio servicos, IPacotesRepositorio pacotes) : IRequestHandler<CriarPacoteCommand, PacoteDetalheResultado>
{
    public async Task<PacoteDetalheResultado> Handle(CriarPacoteCommand request, CancellationToken cancellationToken)
    {
        ...
        var pacote = new Pacote(usuario.EmpresaId, request.Nome, request.Descricao, request.TipoPrecificacao, request.Preco, request.ServicoIds);
...
internal sealed class AtualizarPacoteHandler(IUsuarioContexto usuario, IServicosRepositorio servicos, IPacotesRepositorio pacotes) : IRequestHandler<AtualizarPacoteCommand, PacoteDetalheResultado>
{
    public async Task<PacoteDetalheResultado> Handle(AtualizarPacoteCommand request, CancellationToken cancellationToken)
    {
        ...
        pacote.Atualizar(request.Nome, request.Descricao, request.TipoPrecificacao, request.Preco, request.ServicoIds);
