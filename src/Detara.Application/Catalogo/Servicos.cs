...
public sealed record ServicoListaItemResultado(Guid Id, string Nome, Guid CategoriaServicoId, string CategoriaNome, decimal? PrecoBase, int? DuracaoEstimadaMinutos, bool EhAtivo);
public sealed record ServicoDetalheResultado(Guid Id, Guid CategoriaServicoId, string CategoriaNome, string Nome, string? Descricao, decimal? PrecoBase, int? DuracaoEstimadaMinutos, int Ordem, DateTime CriadoEmUtc, DateTime? AtualizadoEmUtc, bool EhAtivo);
public sealed record ServicoSelecaoResultado(Guid Id, string Nome, string CategoriaNome, decimal? PrecoBase, int? DuracaoEstimadaMinutos, bool EhAtivo);
...
public sealed record CriarServicoCommand(Guid CategoriaServicoId, string Nome, string? Descricao, decimal? PrecoBase, int? DuracaoEstimadaMinutos, int Ordem) : IRequest<ServicoDetalheResultado>;
public sealed record AtualizarServicoCommand(Guid Id, Guid CategoriaServicoId, string Nome, string? Descricao, decimal? PrecoBase, int? DuracaoEstimadaMinutos, int Ordem) : IRequest<ServicoDetalheResultado>;
...
{
    protected void Regras(Func<T, Guid> categoria, Func<T, string> nome, Func<T, string?> descricao, Func<T, decimal?> preco, Func<T, int?> duracao, Func<T, int> ordem)
    {
        RuleFor(x => categoria(x)).NotEmpty(); RuleFor(x => nome(x)).NotEmpty().MinimumLength(2).MaximumLength(160);
        RuleFor(x => descricao(x)).MaximumLength(2000); RuleFor(x => preco(x)).GreaterThanOrEqualTo(0).When(x => preco(x).HasValue);
        RuleFor(x => duracao(x)).InclusiveBetween(1, 43200).When(x => duracao(x).HasValue); RuleFor(x => ordem(x)).GreaterThanOrEqualTo(0);
    }
}
internal sealed class CriarServicoValidator : ServicoValidatorBase<CriarServicoCommand> { public CriarServicoValidator() => Regras(x => x.CategoriaServicoId, x => x.Nome, x => x.Descricao, x => x.PrecoBase, x => x.DuracaoEstimadaMinutos, x => x.Ordem); }
internal sealed class AtualizarServicoValidator : ServicoValidatorBase<AtualizarServicoCommand> { public AtualizarServicoValidator() { RuleFor(x => x.Id).NotEmpty(); Regras(x => x.CategoriaServicoId, x => x.Nome, x => x.Descricao, x => x.PrecoBase, x => x.DuracaoEstimadaMinutos, x => x.Ordem); } }
...
internal sealed class CriarServicoHandler(IUsuarioContexto usuario, ICategoriasServicoRepositorio categorias, IServicosRepositorio servicos) : IRequestHandler<CriarServicoCommand, ServicoDetalheResultado>
{
    public async Task<ServicoDetalheResultado> Handle(CriarServicoCommand request, CancellationToken cancellationToken)
    {
        ...
        var servico = new Servico(usuario.EmpresaId, request.CategoriaServicoId, request.Nome, request.Descricao, request.PrecoBase, request.DuracaoEstimadaMinutos, request.Ordem);
...
internal sealed class AtualizarServicoHandler(IUsuarioContexto usuario, ICategoriasServicoRepositorio categorias, IServicosRepositorio servicos) : IRequestHandler<AtualizarServicoCommand, ServicoDetalheResultado>
{
    public async Task<ServicoDetalheResultado> Handle(AtualizarServicoCommand request, CancellationToken cancellationToken)
    {
        ...
        servico.Atualizar(request.CategoriaServicoId, request.Nome, request.Descricao, request.PrecoBase, request.DuracaoEstimadaMinutos, request.Ordem);
