...
public sealed record AlterarStatusAgendamentoCommand(Guid Id, StatusAgendamento Status, string? MotivoCancelamento) : IRequest<AgendamentoDetalheVisualizacao>;
...
public sealed record ContextoAgendaVisualizacao(string FusoHorario, DateOnly HojeLocal);
...
internal sealed class AlterarStatusAgendamentoValidator : AbstractValidator<AlterarStatusAgendamentoCommand> { public AlterarStatusAgendamentoValidator() { RuleFor(x => x.Id).NotEmpty(); RuleFor(x => x.Status).IsInEnum(); RuleFor(x => x.MotivoCancelamento).MaximumLength(1000); } }
...
internal sealed class AlterarStatusAgendamentoHandler(IUsuarioContexto usuario, IAgendaRepositorio agenda, ICatalogoAgendaConsulta catalogo, IFusoHorarioEmpresaConsulta fusos, IConversorFusoHorario conversor) : IRequestHandler<AlterarStatusAgendamentoCommand, AgendamentoDetalheVisualizacao>
{
    public async Task<AgendamentoDetalheVisualizacao> Handle(AlterarStatusAgendamentoCommand request, CancellationToken ct) { var entidade = await agenda.ObterParaAlteracaoAsync(request.Id, ct) ?? throw new RecursoNaoEncontradoException("Agendamento não encontrado."); entidade.AlterarStatus(request.Status, request.MotivoCancelamento); await agenda.SalvarAsync(ct); return await AgendaFluxo.ObterDetalheAsync(entidade.Id, usuario.EmpresaId, agenda, catalogo, fusos, conversor, ct); }
}
...
internal sealed class ObterContextoAgendaHandler(IUsuarioContexto usuario, IFusoHorarioEmpresaConsulta fusos, IConversorFusoHorario conversor) : IRequestHandler<ObterContextoAgendaQuery, ContextoAgendaVisualizacao> { public async Task<ContextoAgendaVisualizacao> Handle(ObterContextoAgendaQuery request, CancellationToken ct) { var fuso = await AgendaFluxo.ObterFusoAsync(fusos, usuario.EmpresaId, ct); return new(fuso, DateOnly.FromDateTime(conversor.ParaLocal(DateTime.UtcNow, fuso))); } }
