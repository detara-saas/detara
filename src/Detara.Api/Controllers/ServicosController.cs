...
public sealed class ServicosController(ISender sender) : ControllerBase
{
    ...
    public async Task<ActionResult<RespostaApi<IReadOnlyCollection<ServicoSelecaoResponse>>>> Selecao([FromQuery] bool incluirInativos = false, CancellationToken ct = default)
    {
        ...
        return Ok(RespostaApi<IReadOnlyCollection<ServicoSelecaoResponse>>.Ok(itens.Select(x => new ServicoSelecaoResponse(x.Id, x.Nome, x.CategoriaNome, MapearTipo(x.TipoPrecificacao), x.PrecoBase, x.DuracaoEstimadaMinutos, x.EhAtivo)).ToArray()));
    }

    [HttpGet("{id:guid}"), Authorize(Policy = Permissoes.ServicosVisualizar)]
    public async Task<ActionResult<RespostaApi<ServicoDetalheResponse>>> Obter(Guid id, CancellationToken ct)
    {
        var resultado = await sender.Send(new ObterServicoQuery(id), ct);
        return Ok(RespostaApi<ServicoDetalheResponse>.Ok(MapearDetalhe(resultado.Servico, resultado.Execucoes)));
    }
    ...
    {
        var item = await sender.Send(new CriarServicoCommand(request.CategoriaServicoId, request.Nome, request.Descricao, MapearTipo(request.TipoPrecificacao), request.PrecoBase, request.DuracaoEstimadaMinutos, request.Ordem), ct);
    ...
    {
        var item = await sender.Send(new AtualizarServicoCommand(id, request.CategoriaServicoId, request.Nome, request.Descricao, MapearTipo(request.TipoPrecificacao), request.PrecoBase, request.DuracaoEstimadaMinutos, request.Ordem), ct);
    ...
    private static ServicoListaResponse MapearLista(ServicoListaItemResultado x) => new(x.Id, x.Nome, x.CategoriaServicoId, x.CategoriaNome, MapearTipo(x.TipoPrecificacao), x.PrecoBase, x.DuracaoEstimadaMinutos, x.EhAtivo);
    private static ServicoDetalheResponse MapearDetalhe(ServicoDetalheResultado x,
        IReadOnlyCollection<ExecucaoItemCatalogoResultado> execucoes) => new(x.Id, x.CategoriaServicoId,
        x.CategoriaNome, x.Nome, x.Descricao, MapearTipo(x.TipoPrecificacao), x.PrecoBase,
        x.DuracaoEstimadaMinutos, x.Ordem, x.CriadoEmUtc, x.AtualizadoEmUtc, x.EhAtivo,
        execucoes.Select(MapearExecucao).ToArray());
    internal static ExecucaoItemCatalogoResponse MapearExecucao(ExecucaoItemCatalogoResultado x) => new(
        x.OrdemServicoId, x.OrdemServicoCodigo, x.ExecutadaEmUtc, x.ClienteNome, x.VeiculoDescricao,
        x.VeiculoPlaca, x.ValorUnitario, x.Quantidade,
        (Detara.Contracts.Atendimento.StatusOrdemServicoContrato)(int)x.Status);
