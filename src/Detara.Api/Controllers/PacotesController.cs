...
public sealed class PacotesController(ISender sender) : ControllerBase
{
    ...
    public async Task<ActionResult<RespostaApi<PacoteDetalheResponse>>> Obter(Guid id, CancellationToken ct)
    {
        var resultado = await sender.Send(new ObterPacoteQuery(id), ct);
        return Ok(RespostaApi<PacoteDetalheResponse>.Ok(MapearDetalhe(resultado.Pacote, resultado.Execucoes)));
    }
    ...
    private static PacoteDetalheResponse MapearDetalhe(PacoteDetalheResultado x,
        IReadOnlyCollection<ExecucaoItemCatalogoResultado> execucoes) => new(x.Id, x.Nome,
        x.Descricao, MapearTipo(x.TipoPrecificacao), x.Preco, x.SomaServicos, x.Economia,
        x.DuracaoEstimadaMinutos, x.CriadoEmUtc, x.AtualizadoEmUtc, x.EhAtivo,
        x.Servicos.Select(s => new PacoteServicoResponse(s.ServicoId, s.Nome, s.CategoriaNome,
            MapearTipo(s.TipoPrecificacao), s.PrecoBase, s.DuracaoEstimadaMinutos, s.Ordem,
            s.EhAtivo)).ToArray(), execucoes.Select(ServicosController.MapearExecucao).ToArray());
