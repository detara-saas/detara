...
[[ "${1:-}" == --confirm-disposable ]] || { echo 'Exige --confirm-disposable.' >&2; exit 2; }
shift
...
[[ "$backup_dir" == /var/opt/mssql/backups && "$data_dir" == /var/opt/mssql/data ]] || { echo 'Diretório fora do padrão.' >&2; exit 2; }
...
existing="$("$sqlcmd" -S "${SQLCMDHOST:-localhost}" -U "${SQLCMDUSER:-sa}" -C -b -h -1 -W -Q "SET NOCOUNT ON; SELECT COUNT(*) FROM sys.databases WHERE name=N'$target_database'")"
[[ "${existing//[[:space:]]/}" == 0 ]] || { echo 'Banco de destino já existe; nenhuma alteração realizada.' >&2; exit 2; }

created=false
cleanup() {
  [[ "$created" == true ]] || return 0
  "$sqlcmd" -S "${SQLCMDHOST:-localhost}" -U "${SQLCMDUSER:-sa}" -C -b \
    -Q "IF DB_ID(N'$target_database') IS NOT NULL BEGIN ALTER DATABASE [$target_database] SET SINGLE_USER WITH ROLLBACK IMMEDIATE; DROP DATABASE [$target_database]; END" || { echo 'Falha ao limpar banco descartável; intervenção necessária.' >&2; return 1; }
}
trap cleanup EXIT
